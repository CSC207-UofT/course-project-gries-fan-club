...
public class IngredientMatcher extends AbstractMatcher {
     ...
     */
    public List<Recipe> recipesMatched(RecipeStorage recipes, int numRecipes) {
        numRecipes = Math.min(numRecipes, recipes.recipes().len()); // Ensure that numRecipes < recipes in storage
        List<Recipe> allRecipes = new ArrayList<>();

        for (Recipe recipe : recipes.recipes()) {
            // if it's less than numberOfRecipes, just add it
            if (allRecipes.size() < numRecipes)
